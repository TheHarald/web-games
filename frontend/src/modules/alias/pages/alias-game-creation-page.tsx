...
import { typedFetch } from "@/src/requests/fetch-utils";
import { TAliasWord } from "../services/alias-reducer";
...
export function AliasGameCreationPage() {
  ...
  useEffect(() => {
    typedFetch<{}, TAliasWord[]>(
      "http://localhost:3001/alias/words",
      "GET"
    ).then((words) => {
      dispatch({
        type: ALIAS_GAME__SET_GAME_SESSION_WORDS,
        words,
      });
      dispatch({
